...
// Helper to recalculate ContentTask target progress
const recalculateSeoTargetProgress = async (employeeId, taskType) => {
  if (!employeeId) return;
  try {
    const template = await ContentActivityTemplate.findOne({ activityName: taskType });
    if (!template) return;
    ...
    const targets = await EmployeeTarget.find({
      ...
      targetType: 'ContentTask',
    });

    for (const target of targets) {
      // Count completed ContentTasks for this employee & type within the target's start and end dates
      const completedCount = await ContentTask.countDocuments({
        employeeId,
        type: taskType,
        status: 'completed',
      ...
      await target.save();
      console.log(`Recalculated ContentTask target ${target._id}: currentValue=${target.currentValue}, status=${target.status}`);
    }
  } catch (error) {
    console.error('Error recalculating ContentTask target progress:', error);
  }
};

// @desc    Get all ContentTask tasks based on filters
// @route   GET /api/seo-tasks
// @access  Public
export const getContentTasks = async (req, res) => {
  try {
    ...
    const tasks = await ContentTask.find(query).sort({ createdAt: -1 });

    res.status(200).json({ success: true, count: tasks.length, data: tasks });
  } catch (error) {
    console.error('Error fetching ContentTask tasks:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};

// @desc    Create a new ContentTask task
// @route   POST /api/seo-tasks
// @access  Public
export const createContentTask = async (req, res) => {
  try {
    ...
    if (employeeId) {
      ...
      const notificationData = {
        title: 'New ContentTask Task Assigned',
        message: `You have been assigned a new ContentTask task: ${title}`,
        category: 'task',
        severity: 'info',
        userId: employeeId,
      };
    ...
    console.error('Error creating ContentTask task:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};

// @desc    Update an ContentTask task
// @route   PUT /api/seo-tasks/:id
// @access  Public
export const updateContentTask = async (req, res) => {
  try {
    const originalTask = await ContentTask.findById(req.params.id);
    ...
    if (originalTask.status !== 'completed' && updatedTask.status === 'completed') {
      ...
      const notifications = managers.map(manager => {
        const notif = {
          title: 'ContentTask Task Completed',
          message: `${employeeName} completed the ContentTask task: ${updatedTask.title}`,
          category: 'seo',
          severity: 'success',
          userId: manager._id
        };
    ...
    console.error('Error updating ContentTask task:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};

// @desc    Delete an ContentTask task
// @route   DELETE /api/seo-tasks/:id
// @access  Public
export const deleteContentTask = async (req, res) => {
  try {
    const task = await ContentTask.findById(req.params.id);

    if (!task) {
      return res.status(404).json({ success: false, message: 'Task not found' });
    }

    await ContentTask.findByIdAndDelete(req.params.id);
    ...
    console.error('Error deleting ContentTask task:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};
