...
// Helper to recalculate SalesTask target progress
const recalculateSeoTargetProgress = async (employeeId, taskType) => {
  if (!employeeId) return;
  try {
    const template = await SalesActivityTemplate.findOne({ activityName: taskType });
    if (!template) return;
    ...
    const targets = await EmployeeTarget.find({
      ...
      targetType: 'SalesTask',
    });

    for (const target of targets) {
      // Count completed SalesTasks for this employee & type within the target's start and end dates
      const completedCount = await SalesTask.countDocuments({
        employeeId,
        type: taskType,
        status: 'completed',
      ...
      await target.save();
      console.log(`Recalculated SalesTask target ${target._id}: currentValue=${target.currentValue}, status=${target.status}`);
    }
  } catch (error) {
    console.error('Error recalculating SalesTask target progress:', error);
  }
};

// @desc    Get all SalesTask tasks based on filters
// @route   GET /api/seo-tasks
// @access  Public
export const getSalesTasks = async (req, res) => {
  try {
    ...
    const tasks = await SalesTask.find(query).sort({ createdAt: -1 });
    ...
    console.error('Error fetching SalesTask tasks:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};

// @desc    Create a new SalesTask task
// @route   POST /api/seo-tasks
// @access  Public
export const createSalesTask = async (req, res) => {
  try {
    ...
    if (employeeId) {
      ...
      const notificationData = {
        title: 'New SalesTask Task Assigned',
        message: `You have been assigned a new SalesTask task: ${title}`,
        category: 'task',
        severity: 'info',
        userId: employeeId,
      };
    ...
    console.error('Error creating SalesTask task:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};

// @desc    Update an SalesTask task
// @route   PUT /api/seo-tasks/:id
// @access  Public
export const updateSalesTask = async (req, res) => {
  try {
    const originalTask = await SalesTask.findById(req.params.id);
    ...
    if (newUpdate || hasLegacyQuantityUpdate) {
      ...
      updatedTask = await SalesTask.findByIdAndUpdate(req.params.id, taskFields, { new: true, runValidators: true });
    }
    ...
    if (originalTask.status !== 'completed' && updatedTask.status === 'completed') {
      ...
      const notifications = managers.map(manager => {
        const notif = {
          title: 'SalesTask Task Completed',
          message: `${employeeName} completed the SalesTask task: ${updatedTask.title}`,
          category: 'seo',
          severity: 'success',
          userId: manager._id
        };
    ...
    console.error('Error updating SalesTask task:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};

// @desc    Delete an SalesTask task
// @route   DELETE /api/seo-tasks/:id
// @access  Public
export const deleteSalesTask = async (req, res) => {
  try {
    ...
    await SalesTask.findByIdAndDelete(req.params.id);
    ...
    console.error('Error deleting SalesTask task:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};
