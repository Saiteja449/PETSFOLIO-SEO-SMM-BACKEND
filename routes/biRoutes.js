  ...
  try {
    ...
    if (combinedText.includes("task")) {
      ...
      const [smm, seo, content, sales, creative] = await Promise.all([
        SmmTask.find().populate("employeeId", "name").lean(),
        SeoTask.find().populate("employeeId", "name").lean(),
        ContentTask.find().populate("employeeId", "name").lean(),
        SalesTask.find().populate("employeeId", "name").lean(),
        CreativeTask.find().populate("employeeId", "name").lean()
      ]);
